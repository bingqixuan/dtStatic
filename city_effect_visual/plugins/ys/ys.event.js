...
function dbclick(el,callback,useCapture) {
    isMobile() ? tap(el, callback, useCapture) : el.addEventListener('dbclick',e=> callback(e), useCapture)
}
...
HTMLElement.prototype.listen = function (event,callback,useCapture) {
    switch (event) {
        // 移动端
        case 'tap':
